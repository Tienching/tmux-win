...
Additional Linux surface parity evidence from 2026-05-15:
...
`windows/linux-behavior-parity.ps1` passed 140 matched Windows/Linux cases for
...
loading, key binding bind/list/unbind, `select-layout`,
`run-shell -C`, `run-shell -b`, `if-shell`, `pipe-pane -O/-I`, hook execution, session rename,
...
| Clipboard | Smoke covers native Windows clipboard set/get, `refresh-client -l`, and OSC 52 pane clipboard updates. Native clipboard open retries handle transient ownership. | Clipboard format variants beyond text and hostile clipboard owner timing. |
| Configuration, parser, formats, hooks | Smoke covers startup `-f`, `source-file`, Windows `%VAR%` and wildcard config expansion, hooks, parser Windows paths, and environment behavior. `windows/config-parser-stress.ps1` adds semicolon-separated config commands, explicit nested `source-file`, `%ENV%` source globs, paths with spaces and shell metacharacters, hooks, `if-shell`, key bindings, and format-bearing option values. `windows/audit-command-surface.ps1` checks command count, required commands, global/server/window options, key-binding count, required key tables, and Windows default option values. `windows/linux-parity-matrix.ps1` compares Linux tmux command/option/key-table surface against Windows and currently reports zero Linux surface items missing on Windows. `windows/linux-behavior-parity.ps1` adds 140 focused WSL/Windows behavior checks for core server state, session groups, `has-session` exit codes, `kill-session`, select-window/last-window/select-pane/last-pane active state, next/previous-window navigation, resize-pane zoom toggling, buffers, paths, formats, configuration, key-bindings, copy-mode, hooks, control mode, and command workflows, including matched global/server/window/user option set/show and option unset/default behavior, environment set/unset and pane inheritance, `source-file` configuration loading, key binding bind/list/unbind, pane current command/path formats, dynamic pane cwd format updates, capture-pane history range, buffer append, buffer save append, and rotate-window pane reordering. | Broader upstream config corpus and fully exhaustive behavior-level option parity against upstream defaults. |
...
- Full signal parity is not complete beyond the current pane `C-c`, ETX,
  ...
  attached raw Ctrl+C smoke coverage.
