...
"@
	...
	$realConsolePowerShellCtrlBreakMarker =
	    "TMUX_WIN32_REAL_CONSOLE_POWERSHELL_CTRL_BREAK"
	$realConsolePowerShellCtrlBreakStarted = Join-Path $Temp `
	    "real-console-powershell-ctrlbreak-started.txt"
	$realConsolePowerShellCtrlBreakInput = Join-Path $Temp `
	    "real-console-powershell-ctrlbreak-input.txt"
	$realConsolePowerShellCtrlBreakExit = Join-Path $Temp `
	    "real-console-powershell-ctrlbreak-exit.txt"
	$realConsolePowerShellCtrlBreakSize = Join-Path $Temp `
	    "real-console-powershell-ctrlbreak-size.txt"
	$realConsolePowerShellCtrlBreakFile = Join-Path $Temp `
	    "real-console-powershell-ctrlbreak-sent.txt"
	$realConsolePowerShellCtrlBreakCommand =
	    '"powershell -NoProfile -Command ""Start-Sleep -Seconds 30"""'
	$realConsolePowerShellCtrlBreakArguments = @("-NoProfile",
	    "-ExecutionPolicy", "Bypass", "-File", $realConsoleProbe,
	    "-Tmux", $Tmux, "-ServerName", $ServerName, "-Session",
	    "attachsmoke", "-Marker",
	    "TMUX_WIN32_REAL_CONSOLE_POWERSHELL_CTRL_BREAK_ATTACH",
	    "-StartedFile", $realConsolePowerShellCtrlBreakStarted,
	    "-InputFile", $realConsolePowerShellCtrlBreakInput,
	    "-ExitFile", $realConsolePowerShellCtrlBreakExit,
	    "-SizeFile", $realConsolePowerShellCtrlBreakSize,
	    "-CtrlBreakCommand", $realConsolePowerShellCtrlBreakCommand,
	    "-CtrlBreakFile", $realConsolePowerShellCtrlBreakFile,
	    "-CtrlBreakMarker", $realConsolePowerShellCtrlBreakMarker)
	$realConsolePowerShellCtrlBreakStart =
	    [System.Diagnostics.ProcessStartInfo]::new()
	$realConsolePowerShellCtrlBreakStart.FileName = "powershell.exe"
	$realConsolePowerShellCtrlBreakStart.Arguments =
	    ($realConsolePowerShellCtrlBreakArguments | ForEach-Object {
		ConvertTo-WindowsArgument $_
	    }) -join " "
	$realConsolePowerShellCtrlBreakStart.UseShellExecute = $false
	$realConsolePowerShellCtrlBreakStart.CreateNoWindow = $true
	$realConsolePowerShellCtrlBreakProcess =
	    [System.Diagnostics.Process]::Start(
	    $realConsolePowerShellCtrlBreakStart)
	$realConsoleWait.Restart()
	while ($realConsoleWait.ElapsedMilliseconds -lt 12000 -and
	    -not (Test-Path -LiteralPath `
	    $realConsolePowerShellCtrlBreakFile)) {
		Start-Sleep -Milliseconds 200
	}
	Assert-FileContains "real console PowerShell Ctrl+Break sent" `
	    $realConsolePowerShellCtrlBreakFile "sent"
	$realConsoleWait.Restart()
	while ($realConsoleWait.ElapsedMilliseconds -lt 15000) {
		$realConsoleCapture = (Invoke-SmokeTmux @(
		    "capture-pane", "-p", "-t", "attachsmoke:0.0")).Out
	...
	Assert-Contains "real console PowerShell Ctrl+Break interrupt" `
	    $realConsoleCapture $realConsolePowerShellCtrlBreakMarker
	$realConsoleCommand = Wait-PaneCurrentCommand `
	    "real console PowerShell Ctrl+Break shell restored" `
	    "attachsmoke:0.0" "cmd.exe" 12000
	Assert-Contains "real console PowerShell Ctrl+Break shell restored" `
	    $realConsoleCommand "cmd.exe"
	try {
		Invoke-SmokeTmux @("detach-client", "-s", "attachsmoke") 5 |
		    Out-Null
	...
	if (-not $realConsolePowerShellCtrlBreakProcess.WaitForExit(10000)) {
		$realConsolePowerShellCtrlBreakProcess.Kill()
		throw "real console PowerShell Ctrl+Break probe did not exit"
	}
	if ($realConsolePowerShellCtrlBreakProcess.ExitCode -ne 0) {
		throw ("real console PowerShell Ctrl+Break probe exited with " +
		    "{0}" -f `
		    $realConsolePowerShellCtrlBreakProcess.ExitCode)
	}
	Assert-FileContains "real console PowerShell Ctrl+Break exit" `
	    $realConsolePowerShellCtrlBreakExit "0"
	Write-Pass "real console attach PowerShell Ctrl+Break"
