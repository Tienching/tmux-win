...
The audit also records the local workflow file hash and the `origin` branch
state so a `missing_workflow`, `branch_not_pushed`, or `remote_head_mismatch`
result can be distinguished from a real hosted workflow failure.
